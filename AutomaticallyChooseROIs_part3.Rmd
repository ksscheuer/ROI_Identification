...
import pandas
pixel_cluster_data = r.pixel_cluster_data #cluster colors
...
def check_cell(x, y, group_index, color):
  ...
  print ("x is",x,", y is",y,", group_index is",group_index,", color is",color)
  print ("cell_value is",cell_value)
  print ("cluster_results[y][x] is",cluster_results[y][x])
  # print ("cell_value != 0 is ",cell_value != 0)
  # print ("cluster_results[y][x] == 0 is ",cluster_results[y][x] == 0)
  # print ("color == 0 or cell_value == color is ",(color == 0 or cell_value == color))
  # print (cell_value != 0 and cluster_results[y][x] == 0 and (color == 0 or cell_value == color))
  if (cell_value != 0 and cluster_results[y][x] == 0 and (color == 0 or cell_value == color)):
    cluster_results[y][x] = group_index
    # print (cluster_results[y][x])
    my_bound = [x,x,y,y]
    xleft = max(0, x-1)
    ...
    # print ("child_bound input values are",x,yup,group_index,cell_value)
    # print ("child_bound is",child_bound)
...
check_cell(25,64,group_index,0)
...
plt.matshow(cluster_results)
plt.show()
print (numpy.unique(cluster_results))

plt.matshow(pixel_cluster_data)
plt.show()
print (numpy.unique(pixel_cluster_data))

print ((max(numpy.unique(pixel_cluster_data)))+1)
print (max(numpy.unique(pixel_cluster_data))+1)
...
roi_averages = roi_averages * (cluster_results > 0)
...
for i in range(0,width): #add electrode as new "cluster"
  for j in range(0,height):
    if electrode_data[j][i] != 0 and roi_averages[j][i] == 0:
      # roi_averages[j][i] = (max(numpy.unique(roi_averages))+1)
      roi_averages[j][i] = 10
