...
unique_SNR = numpy.delete(test,0)
# print (unique_SNR)
...
final_rois$Y <- final_rois_from_python[,2]
...
for (i in 0:max(final_rois$ROI_ID)) {
  ...
  dat_file[c((row_id+3):(row_id+3+length(which(final_rois$ROI_ID==i))-1)),1] <- final_rois$Pixel_ID[which(final_rois$ROI_ID==i)] 
  row_id <- row_id+2+length(which(final_rois$ROI_ID==i))
}
