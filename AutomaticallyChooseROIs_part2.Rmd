...
knitr::opts_knit$set(root.dir = "G:/My Drive/Jackson/PhotoZ_data/Step1_ROIs/from Wen-chi/prox/02-01")
...
ROI_diameter_cutoff <- 3 #max number of pixels for diameter of ROI
  #ROI_diameter_cutoff = 2 for 10x lens and 3 for 20x lens
myslice <- "02-01"
mydate <- "Wen-chi, Prox-1"
SNRcutoff <- 4
Ampcutoff <- 0.001
...
```
``` {r fig.width=7, fig.height=6}
################## Remove cluster 1 and replot #############################
############################################################################

cluster_to_remove <- which(clusteraverages_all[,1]==min(clusteraverages_all[,1]))

myfiles_df_cl_noclust1 <- myfiles_df_cl
clusteraverages_noclust1 <- clusteraverages_all[-cluster_to_remove,]
myfiles_df_cl_noclust1$Cluster[which(myfiles_df_cl_noclust1$Cluster==cluster_to_remove)] <- NA

ggplot(myfiles_df_cl_noclust1,aes(x=X,y=Y)) +
  geom_tile(aes(fill=myfiles_df_cl_noclust1$Cluster),color="gray") +
  labs(title=paste("Step 2. Clusters"),subtitle=paste("Date: ",mydate,", Slice: ",myslice,", Clusters: ",k,sep=""), fill="SNR Avg +/- SD") +
  theme(
    axis.title.y = element_blank(),
    axis.title.x = element_blank(),
    ...
    plot.subtitle = element_text(hjust=0.5),
    legend.position = "none") +
  scale_fill_gradient(na.value="lightgray") +
  # ) +
  # guides(fill = guide_legend(keyheight = 0.9)) 
  scale_y_reverse() 
  # scale_fill_gradientn(breaks=seq(1:k),
  #                      labels=c(paste(round(clusteraverages_noclust1$Avg,3),"+/-",round(clusteraverages_noclust1$SD,3))),
  #                      colors=rev(c("red1","yellow1","green1","dodgerblue1","navy"))
  # )

# ggsave("Step2b_Cluster.jpg",height=6,width=7.5)
...
pixel_cluster_data <- t(matrix(myfiles_df_cl_noclust1$Cluster, ncol=width, nrow=height))
pixel_cluster_data[is.na(pixel_cluster_data)] <- 0
pixel_sn_data <- t(matrix(myfiles_df_cl_noclust1$Avg, ncol=width, nrow=height))
pixel_amp_data <- t(matrix(ampfiles$Avg, ncol=width, nrow=height))

myfiles_df_cl_noclust1_with_electrode <- mutate(myfiles_df_cl_noclust1,Electrode = 0)
for (i in 1:nrow(electrodepixels)) {
  myfiles_df_cl_noclust1_with_electrode$Electrode[which(myfiles_df_cl_noclust1_with_electrode$PixelID==electrodepixels[i,1])] <- 1
}
electrode_data <- t(matrix(myfiles_df_cl_noclust1_with_electrode$Electrode, ncol=width, nrow=height))

electrode_x_min <- min(myfiles_df_cl_noclust1_with_electrode$X[which(myfiles_df_cl_noclust1_with_electrode$Electrode==1)])
electrode_tip_x <- max(myfiles_df_cl_noclust1_with_electrode$X[which(myfiles_df_cl_noclust1_with_electrode$Electrode==1)])
electrode_y_min <- min(myfiles_df_cl_noclust1_with_electrode$Y[which(myfiles_df_cl_noclust1_with_electrode$Electrode==1)])
electrode_y_max <- max(myfiles_df_cl_noclust1_with_electrode$Y[which(myfiles_df_cl_noclust1_with_electrode$Electrode==1)])
...
plt.title("Date: %s, Slice: %s, SNR Cutoff: %d" % (mydate,myslice,SNRcutoff),fontsize=13)
plt.show()
...
roi_amp_averages = numpy.zeros((width,height))
# roi_amp_averages = roi_averages

for roi_group_index in numpy.unique(cluster_results):
  location_index = (cluster_results == roi_group_index)
  amp_data_for_group = pixel_amp_data * location_index
  average = amp_data_for_group[amp_data_for_group != 0].mean()
  if average > Ampcutoff:
    print (average)
  else:
...
unique, counts = numpy.unique(roi_amp_averages, return_counts=True)
...
roi_amp_averages[roi_amp_averages < Ampcutoff] = 0
r.roi_amp_average = roi_amp_averages
...
plt.title("Date: %s, Slice: %s, Amplitude Cutoff: %d" % (mydate,myslice,Ampcutoff),fontsize=13)
plt.show()
plt.savefig("Step7b_ROIs_with_Amp_Greater_than_Cutoff.jpg")

roi_keep = numpy.zeros((width,height))
for i in range(0,width): 
  for j in range(0,height):
...
plt.title("Date: %s, Slice: %s, SNR Cutoff: %d, Amp Cutoff: %1.3f" % (mydate,myslice,SNRcutoff,Ampcutoff),fontsize=13)
plt.show()
...
plt.title("Date: %s, Slice: %s, SNR Cutoff: %d, Amplitude Cutoff: %1.3f" % (mydate,myslice,SNRcutoff,Ampcutoff),fontsize=13)
plt.show()
...
unique_SNR = numpy.unique(roi_keep)
unique_SNR = numpy.delete(unique_SNR,0)

roi_keep = numpy.transpose(roi_keep)

pixel_coords_for_all_rois = []
total_pixels_in_all_rois = 0
for roi in range(0,len(unique_SNR)):
  pixel_coords_for_roi = list(zip(*numpy.where(roi_keep == unique_SNR[roi])))
...
for (i in 1:nrow(final_rois)) {
  all_x <- which(myfiles_df_cl_noclust1$X==final_rois$X[i])
  all_y <- which(myfiles_df_cl_noclust1$Y==final_rois$Y[i])
